...
ReqGrid <- read.csv("B1_ReqGrid.csv")
ReqGrid <- as.numeric(ReqGrid$x)

SoFittedMean <- read.csv("B1_SoFittedMean.csv")
SoFittedMean <- apply(SoFittedMean, 2, as.numeric)

SoEval <- read.csv("B1_SoEval.csv")
SoEval <- as.numeric(SoEval$x)

SoEfunc <- read.csv("B1_SoEfunc.csv")
SoEfunc <- apply(SoEfunc, 2, as.numeric)

SoErrvar <- read.csv("B1_SoErrvar.csv")
